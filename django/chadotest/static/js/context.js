...
var family_names = {};
for( var i = 0; i < context_data.families.length; i++ ) {
	var fam = context_data.families[i];
            ...
            success: function(data) {
				...
				if( d.family.length > 0 ) {
					for( var i = 0; i < d.family.length; i++ ) {
						var id = d.family[i];
						html += '<li><a href="'+phylo_link+id+'/">'+family_names[id.toString()]+'</a></li>';
					}
				} else {
					html += '<li>No families</li>';
				}
				html += '</ul>';
...
legend.append("text")
    ...
    .text(function(d) {
        ...
        for( var i = 0; i < d.length; i++ ) {
			fams.push( family_names[ d[i].toString() ] );
        }
        return fams.join();
    });
